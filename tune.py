"""
Bounded Optuna hyperparameter search for the v3 CLV models.

Tunes one (model-family, dataset) config at a time on the corrected objective
(scheduled-sampling dose + Kendall/loss weighting + capacity + optimisation).
The scheduled-sampling smoke showed the bias is a tunable knob in [-50%, +45%];
this search finds the dose (and the rest) that lands it near zero WITHOUT
collapsing per-customer discrimination (the objective explicitly guards the
degenerate "bias≈0 but gini≈0" trap observed in the 25-epoch smoke).
...
* Evaluated in `sample` mode (the primary/headline decode) with a reduced
  n_scenarios for trial speed; the final multi-seed run uses the full 200.
...
def main() -> None:
    p = argparse.ArgumentParser(description="Bounded Optuna HPO for a v3 CLV config.")
